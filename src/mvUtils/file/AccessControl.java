...
public class AccessControl {
    ...
    public AccessControl(String filePath) throws Exception {
        this(filePath, false);
    }

    public AccessControl(String filePath, boolean onlyIfExists) throws Exception {
        this(filePath, null, null, onlyIfExists);
    }

    public AccessControl(String filePath, String passwordRegx, String passwordToolTip, boolean onlyIfExists) throws Exception {
    ...
    public StatusWithMessage getAndSaveNewAccess(String accessString, String title) {
        ...
        PasswordDialog pDlg;
        pDlg = new PasswordDialog(title, regx, passwordToolTip);
        if (pDlg.allOK) {
    ...
    public StatusWithMessage addNewAccess(String accessString, String name, String password) {
        StatusWithMessage retVal = new StatusWithMessage();
        String nameHash =  getHash((accessString + name).toLowerCase());
        String passWordHash = getHash(accessString + name + password);
    ...
    public static void main(String[] args) {
        ...
        try {
            AccessControl access = new AccessControl(path);
            StatusWithMessage st1 = access.addNewAccess("EXPERT", "vishuNew", "pass001");
            System.out.println("" + st1.getDataStatus() + ":" + st1.getErrorMessage());
            boolean isOK = access.checkPassword("EXPERT", "mv1", "Vis0001$") ;
            System.out.println("EXPERT" + ":mv1" +  ":Vis0001$ " + st1.getDataStatus() + ":" + isOK);
            ...
            System.out.println((st4.getDataStatus() == StatusWithMessage.DataStat.OK) ? "Acess Deleted" : st4.getErrorMessage());
            StatusWithMessage st3 = access.getAndSaveNewAccess("RUTIME", "For Access as Runtime");
            System.out.println((st3.getDataStatus() == StatusWithMessage.DataStat.OK) ? "new password is ok" : st3.getErrorMessage());
            StatusWithMessage st5 = access.getAndCheckPassword("RUTIME", "Access as RUTIME");
